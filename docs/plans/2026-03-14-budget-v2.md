...
function migrateV28(db: DatabaseSync): void {
  ...
  db.prepare(`
    CREATE INDEX IF NOT EXISTS idx_cost_records_goal
...
export function checkBudgetV2(
  ...
  const remaining = Math.min(
    ...
    params.agentId ? (checkBudgetRow(params.projectId, params.agentId, db)?.remaining ?? Infinity) : Infinity,
  );
...
- `reserveBudget(planId, projectId, cents, tokens, db)` — increments `reserved_cents` and `reserved_tokens`
- `settlePlanItem(projectId, estimatedCents, estimatedTokens, db)` — decrements reservation by estimated amounts (MAX 0)
- `releasePlanReservation(projectId, remainingCents, remainingTokens, db)` — releases remaining reservation
